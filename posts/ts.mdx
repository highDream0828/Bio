...
Writing function is a piece of cake when you know the JavaScript but it gets a little bit complex in typescript. Don’t worry, we will take every aspect of that. We need to define the types of two things in function **Parameters** & **Return Types.**
...
signUp({ email: "hello@Bio", password: "12345678", name: "Johnny" }); 
...
let newUser = { email: "hello@Bio", password: "12345678", name: "Johnny" };
signUp(newUser);
```
...
// ❌ ERROR: Property 'x' has no initializer and is not assigned in the constructor
```
...
// ❌ ERROR: Type 'string' & 'boolean' are not assignable to type 'number'
coordinates.x = "11";
coordinates.y = false;
```
...
// ❌ Cannot assign to 'quadrant' because it is a read-only property
...
class Cords {
	x: number;
	y: number;
	quadrant: number; // ❌ ERROR: Property 'quadrant' has no initializer and is not assigned in the constructor.
...
class Cords {
	constructor(x:number, y: number){
		this.x = x; 	// ❌ ERROR: Property 'x' does not exist on type 'Cords'
		this.y = y; 	// ❌ ERROR: Property 'Y' does not exist on type 'Cords'
	}
}
...
You might have heard that before about `private` and `public` somewhere. `private` doesn’t allow access to the member.
...
class Cords {
	x: number = 0;
	y: number = 0;
	private z: number = 0;  // 👈 private

	constructor(x:number, y: number){
		this.x = x; 	
		this.y = this.y;
		this.z = 10; // ✅ CORRECT: because we can use it inside this class 
	}
}

const cds = new Cords(10, 20);

cds.z; // ❌ ERROR: Property 'z' is private and only accessible within class 'Cords'
```
...
dish1._chef  // ❌ ERROR: Property '_chef' is private and only accessible within class 'Dish'
console.log(dish1.chefName) // ✅ Output: Gordon Ramsay

dish1.updateChefName  = "James Oliver"; // Using `setter` to update the _chef 
console.log(dish1.chefName) // ✅ Output: James Oliver
```
...
`protected` members are only visible to subclasses of the class they’re declared in. When you use `private` then it won't allow access to the member outside its defined class not even subclasses but with `protected` you can access that member in subclasses but not outside that scope:

<CodeTitle lang="ts" title="index.ts" />
```ts
class Parent {
  private firstName: string = "Johnny"; // 👈 PRIVATE
  protected lastName: string = "Deep";  // 👈 PROTECTED
}
...
// ❌ ERROR: Property 'firstName' is private and only accessible within the class 'Parent'
d.firstName; 
// ❌ ERROR: Property 'lastName' is protected and only accessible within class 'Parent' and its subclasses
d.lastName; 
// ✅ CORRECT
d.middleName;
```
...
class Child extends Parent {
	// ❌ ERROR: Property 'firstName' is private in type 'Parent' but not in type 'Child'.
	firstName = "samu"; 	
...
class SubCounter extends Counter {
	updateCounter(){
		// ❌ Property 'increaseCounter' is private and only accessible within class 'Counter'.
		this.increaseCounter()
		this.decreaseCounter(); /// ✅ because protected
	}
}
...
// ❌ Property 'decreaseCounter' is protected and only accessible within class 'Counter' and its subclasses.
c.decreaseCounter()
```
...
// ❌ ERROR: Class 'Snapchat' incorrectly implements interface 'TakePhoto'.
// ❌ ERROR: Type 'Snapchat' is missing the following properties from type 'TakePhoto': exposure, cameraMode, flashLight
...
// EXAMPLE-2
// ❌ ERROR: Property 'clickPhoto' is missing in type 'SnapChat' but required in type 'TakePhoto'.
class Snapchat implements TakePhoto{
	applyFilter(){}
}
```
...
```ts
abstract class TakePhoto {
	abstract clickPhoto():void   // 👈 abstract method
	constructor(
		public exposure: number,
		public cameraMode: string,
	){}
...
// ❌ Error: Non-abstract class 'Snapchat' does not implement inherited abstract member 'clickPhoto' from class 'TakePhoto'
...
Just as in JavaScript, if you have a base class, you’ll need to call `super();` in your constructor body before using any `this.` members. Let's take one example:
...
identityFour<Laptop>({});  // ❌ ERROR: '{}' is missing the properties
identityFour<Laptop>({
  CPU: "intel",
  cores: 16,
  RAM: 12,
...
// ❌ ERROR: 'screenSize' does not exist in type 'Laptop'
...
function isAdminAccount(account: User | Admin) {
  return account.isAdmin; // ❌ roperty 'isAdmin' does not exist on type 'User'
}
```
...
JavaScript has an operator for checking whether or not a value is an “instance” of another value or another class. As you might have guessed, `instanceof` is also a type guard.

<CodeTitle lang="ts" title="index.ts" />
```ts
function logValue(x: Date | string) {
  if (x instanceof Date) { // 👈 using instanceof
...
Most of the examples we’ve looked at so far have focused on narrowing single variables with simple types like `string`, `boolean`, and `number`. While this is common, most of the time in JavaScript we’ll be dealing with slightly more complex structures.
